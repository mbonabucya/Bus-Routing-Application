...
class RouteDatabase {
    ...
    suspend fun updateRoute(current: String){
        val current_time = ""+LocalDateTime.now().year+"/"+LocalDateTime.now().month+"/"+LocalDateTime.now().dayOfMonth+"-"+LocalDateTime.now().hour+"-"+LocalDateTime.now().minute+"-"+LocalDateTime.now().second+""
        val data = hashMapOf(
            "from" to "Town",
            "dest" to "CMU",
            "current" to current,
            "current_time" to current_time,
            "departure_time" to "Departure time"
